...
const allowlist: AllowlistEntry[] = [
  ...
  // FNXC:EngineProcessRules 2026-08-07-00:00: re-pinned drifted line numbers again for the same
  // unchanged git-plumbing call-sites (self-healing.ts 4215/4221/4258/12839 -> 4305/4311/4348/12929,
  ...
  // call-sites (self-healing.ts 4305/4311/4348/12933 -> 4308/4314/4351/12949 -> 4310/4316/4353/12986
  // -> 4313/4319/4356/13018) after the campaign-scoped branch-name change added comment and candidate
  // lines above them, then the in-review rebinder took its custody base from the campaign seal
  // instead of the mutable task row, adding a multi-line import and a per-task resolve above them.
  ...
  { file: "src/self-healing.ts", line: 4313, primitive: "execSync", signature: "const tipSha = String(execSync(`git rev-parse --verify ${shellQuote(branch)}`, {", reason: SHORT_GIT_PLUMBING },
  { file: "src/self-healing.ts", line: 4319, primitive: "execSync", signature: "const uniqueCommitCount = Number.parseInt(String(execSync(`git rev-list --count ${shellQuote(branch)} --not ${shellQuote(\"main\")}`, {", reason: SHORT_GIT_PLUMBING },
  { file: "src/self-healing.ts", line: 4356, primitive: "execSync", signature: "const branchesRaw = String(execSync(\"git branch --list 'fusion/*'\", {", reason: SHORT_GIT_PLUMBING },
  { file: "src/self-healing.ts", line: 13018, primitive: "execSync", signature: "execSync(`git branch -d ${shellQuote(branch)}`, {", reason: SHORT_GIT_PLUMBING },

...
- Retired route tokens and tools — `mcp-run`, `/v0/groups/agy/mcp`, `mcpjungle-agy`, `tool-suite`, `toolgroup-green`, `toolgroup-green-claude`, `toolgroup-green-codex`, `toolgroup-yellow`, `toolgroup-robinhood-readonly`, `toolgroup-robinhood-write-unwired`, `opencode-research`, `opencode-exec`, broker `deepwiki`, `chunkhound-vault`, and Chunkhound — are search/history only. Current DeepWiki is inside `stack-core`; local search routes through Smart Tree, GitNexus, repowise, Semble, jscpd, and exact `rg`/reads as appropriate; other work routes through SSOT/live-proven `stack-core`, `trading-readonly`, `finance-cc`, direct stdio `agy-bridge`, or local/plugin MCPs.
...
- Keep Codex and interactive OpenCode brokered by default through MCPProxy/mcpproxy-go for broad `stack-core`, `trading-readonly`, `task-control`, and `finance-cc`; do not replace them with direct broad MCPJungle routes unless the task asks and live files justify the policy/metadata loss.
- Preserve the deployed CCC `mcpproxy`: the live `:8163` binary self-reports `v0.51.0 (personal)` and uses `/Users/ryanpappal/.mcpproxy-ccc`, not the unrelated Homebrew binary or its default data directory. Do not `brew upgrade`, repoint, or baseline-refresh without version and cutover proof.
- The deployed broker disables `/mcp/all`; it is not a live or break-glass route. Never invent or enable a direct-catalog route against production port `8163`.
...
- Health/drift: use the authenticated `/api/v1/tools` surface, the deployed CCC binary's `tools list` and `upstream list`, active client config, and emitted manifests. The old `/v0/tools` path returns 404, and `mcpproxy status -o json` can show `0` servers while upstreams are connected.

### Current Client Routes

| Client | Current MCP route | Notes |
|---|---|---|
| Claude Code | Direct `stack-core`, direct `trading-readonly`, direct `task-control`, and direct `finance-cc` on main MCPJungle, plus direct stdio `agy-bridge` | No Claude mcpproxy route; Chunkhound retired and removed 2026-07-14 |
| Codex | `mcpproxy` at `http://127.0.0.1:8163/mcp`, direct `mcpjungle-hot`, direct stdio `agy-bridge`, and configured local/plugin MCPs such as `node_repl` | Broker upstreams provide `stack-core`, `trading-readonly`, `task-control`, and `finance-cc`; DeepWiki is inside `stack-core`; AGY consultation tools are direct MCP tools, not an MCPJungle HTTP group |
| OpenCode | Direct `agy-bridge`, `mcpjungle-hot`, and `mcpproxy` | Treat flattened or dynamically selected tool names as ambiguous unless an exact current route map proves canonical server and tool identity |
...
| `finance-cc` | Public market data through yfinance, FRED, and SEC EDGAR; Massive is excluded because no configured key made it functional. Separate from Robinhood `trading-readonly`; Claude reaches this group directly on main MCPJungle, while Codex and interactive OpenCode reach it through the shared broker |
...
When direct `agy-bridge` is visible, Codex may call exact visible tools such as `agy_bridge__delegate`, `agy_bridge__adversarial_review`, and `agy_bridge__follow_up`. If the direct tools are absent, report `ROUTE_UNAVAILABLE`; do not use `mcp-run`, the retired `/v0/groups/agy/mcp` route, or guessed broker names such as `stack-core:delegate`. Use `follow_up` only after a prior call returns a `session_id`.
...
- MCPProxy/mcpproxy-go is the shared Codex and interactive OpenCode broker for broad `stack-core`, `trading-readonly`, `task-control`, and `finance-cc` access. Keep both clients brokered by default; do not replace the broker with direct broad MCPJungle routes unless a task explicitly asks and live files justify the policy/metadata loss.
- Preserve the deployed CCC MCPProxy warning: the live `:8163` host uses the pinned `/Users/ryanpappal/.mcpproxy-ccc` binary, which self-reports `v0.51.0 (personal)`. The unrelated Homebrew binary and its default data directory are not the live broker. Do not `brew upgrade`, repoint, baseline-refresh, or copy candidate claims into live canon without verifying the exact runtime and obtaining the authority required by the target repo.
...
- The deployed broker's normal `/mcp` route may expose authenticated management tools including `upstream_servers`, `quarantine_security`, `search_servers`, and `list_registries`. Use them only with the operator-authenticated session and follow their returned read/write/destructive classification; scoped tokens keep their server-write restrictions. `unquarantine_server` additionally requires the runtime's explicit allow gate.
...
- For health/drift, use the deployed CCC binary's `tools list` and `upstream list`, the authenticated `/api/v1/tools` endpoint, active client config, and the emitted `mcp-install-manifest.json` and `mcp-inventory-manifest.json` when present. The old `/v0/tools` path returns 404; do not trust `mcpproxy status -o json` alone.

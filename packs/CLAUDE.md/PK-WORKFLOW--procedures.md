...
- **Hard Rule —** CrashCartCapital CI compute runs on the M2 Max through dedicated repository-scoped self-hosted runner registrations by default. GitHub Actions may dispatch and report, but before pushing a new or changed workflow, inspect every `runs-on` value and the live repository runner inventory; hosted runners are failures unless Ryan explicitly authorizes them for that repository. New repositories must register an exact M2 runner and create `docs/ci-runtime.md` or keep CI disabled before the first push. Completion proof must name the actual runner that executed each job and keep current availability separate from reboot persistence. Canon: [[30_DEVSTACK/config/REF-HUM-GitHubActionsM2RunnerStandard|GitHub Actions M2 Runner Standard]].

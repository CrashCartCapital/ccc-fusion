...
- Repository role: Ryan's operator-facing fork of Runfusion/Fusion for turning an admitted PRD into a recoverable, proof-gated coding campaign that can use broad dependency-ready parallelism without creating a second control plane.
...
- Start with `/Users/ryanpappal/01_VAULT/KnR-Vault/00_MAIN/01_ActiveProjects/ccc-fusion/START-HERE-ccc-fusion-CurrentAuthorityMap-2026-08-11.md` and `PRJ-AI-ccc-fusion-PRD-v0.3.md`, then read this repo's `docs/ccc-fusion-product.md`, `docs/testing.md`, and the relevant implementation docs. Older PRDs, conversion plans, wave notes, leases, checkpoints, and forward-fill plans are historical evidence only; they do not override v0.3.
- Preserve one authoritative combined candidate for acceptance, but do not confuse one integration authority with one global writer or a linear campaign. Independent dependency-ready tasks should run concurrently, and deterministic integration nodes join accepted results.
...
- Keep the operator experience dashboard-first and plain-language. The CLI remains an administration and recovery surface, not the normal requirement for starting or understanding a campaign.
- Version 1 campaigns own one primary repository. Multiple independent campaigns may run at once; atomic multi-repository campaigns wait for a demonstrated need.
...
- Normal CCC-Fusion leaf-worker execution uses approved OmniRoute endpoints. Routine work and most campaign-orchestrator work should predominantly use MiniMax M3 and GLM 5.2. This project-specific policy is separate from Ryan's interactive Claude Code and Codex preferences.
- Escalate only when evidence says the routine pool cannot clear the task: approved OpenCode Go compute through OmniRoute for Kimi K3 or Qwen 3.8; then limited Antigravity/Gemini Pro or Antigravity-delivered Opus 4.6 when available; then Codex Terra at high or xhigh, Terra at max, and finally Sol at max. Direct Antigravity or Codex use is a logged bounded step-up exception, not a routine leaf-worker route. Direct Anthropic Claude Code is not a CCC-Fusion worker lane. Resolve exact live model IDs at dispatch and record requested and effective routes.
- Current code uses Fusion's embedded Pi agent path. Do not claim OpenCode is a first-class CCC-Fusion harness until an equal-task Pi-versus-OpenCode benchmark proves it. Choose on accepted-result quality, repair rate, completion time, quota use, tool fidelity, cancellation, and resume evidence rather than brand.
- Routine worker packets may include the declared repository, the admitted CCC PRD packet, and task-specific public documentation. They must not include credentials, secrets, clinical data, private financial-account data, unrelated vault contents, or free browsing of Ryan's files. Public research must be task-scoped and source-cited.
...
- Use `scripts/run-ccc-pg-proof.mjs` only with the exact accepted proof mapping. It owns disposable PostgreSQL startup, zero-skip proof reporting, cleanup on pass, and preserved redacted evidence on failure. Never broaden a frozen mapping merely to make review convenient.
...
- Compile the admitted campaign into a real dependency graph. Release every useful dependency-ready task while respecting frozen interfaces, file leases, proof prerequisites, and protected-action gates. Support multiple roots, split work, barriers, multi-parent joins, and deterministic integration.
- There is no arbitrary product-wide worker-count ceiling. Every campaign still has finite safeguards for money, time, disk, memory, provider quota, runaway retries, heavy builds, and integration pressure. Raise or lower concurrency from observed accepted-work throughput and back off when failures, repair load, queueing, resource pressure, or integration delay rises.
- Budget campaign execution for useful completion, not the smallest theoretical pass. Use explicit finite envelopes sized from the task graph with generous headroom for discovery, implementation, verification, and repair; structural minima are admission floors, not adequacy targets. Prefer a longer useful run over arbitrary premature cutoff, and tighten limits only from evidence. Stop early when authority, safety, quota, runaway, or stagnation evidence requires it.
- Use separate capacity pools for lightweight research/review, writers and worktrees, heavy builds/tests, and integration/delivery so one bottleneck does not freeze unrelated work.
- One worker owns each atomic file scope or shared interface at a time. This is a collision rule, not a global writer limit. Non-overlapping domains, research, fixtures, tests, reviews, and other dependency-ready work should proceed concurrently.
- Use a shallow campaign hierarchy when scale warrants it: one campaign orchestrator, campaign-local domain orchestrators, and leaf workers. Domain orchestrators may split admitted work, assign leases, verify results, and report, but may not expand frozen product scope, change protected authority, or silently rewrite shared contracts.
- Durable agent mail carries coordination and evidence references, but mail alone never changes scope, satisfies a dependency, or grants authority. PostgreSQL workflow state and immutable receipts remain authoritative, and messages need durable identity and duplicate protection.
- Before dispatch or integration, verify branch/worktree identity, remotes, protected paths, dependency state, frozen interface hashes where required, live listeners relevant to proof, and the exact campaign authority receipt.
- A blocked task should not stop independent ready work. Interface drift invalidates dependent work until it is re-derived and re-proved against the new interface.
- The future read-only knowledge advisor is parked until the core campaign loop works. Routine workers receive sealed context packets and cannot browse the vault freely.
...
- Measure useful throughput as accepted, integrated, exact-tree-proven work per wall-clock hour. Started workers, attempted tasks, and worker self-ratings do not count.
- Keep a small attempt receipt for model, harness, task type, prompt profile, chunk size, runtime, quota/cost facts, result classification, repair count, and escalation reason. Use this evidence to tune M3 and GLM task size and prompts; do not build a speculative reinforcement-learning system or dashboard-first optimizer.
...
- `autoMerge: false` means `in-review` is terminal unless a valid frozen campaign receipt explicitly authorizes the dedicated terminal delivery agent to perform the named remote action. Ordinary workers, reviewers, and orchestrators never push, create or update pull requests, or merge remotely.
...
- Provider, credential, billing, non-loopback network, publication, release, upstream write, automatic merge, and live/destructive action gates are action-specific. A frozen campaign may preauthorize exact push, pull-request creation or update, and merge actions for the terminal delivery agent after named checks pass; it does not grant broader authority.
...
- The terminal delivery agent must recheck the exact commit, combined tree, checks, target branch, remote head, and unused authority immediately before delivery. Any drift, failed proof, scope change, or receipt mismatch cancels delivery and returns control to campaign orchestration; the agent never fixes forward on the remote.
- When the frozen candidate, target, checks, remote state, and unused authority are unchanged, the terminal delivery agent uses the existing campaign preauthorization without asking Ryan to approve the same action again.
...
- `docs/ccc-fusion-product.md` — current product contract, target architecture, worker-compute policy, operator experience, and implementation truth boundary.
...
- `docs/plans/README.md` — current-plan authority map and historical-plan index.

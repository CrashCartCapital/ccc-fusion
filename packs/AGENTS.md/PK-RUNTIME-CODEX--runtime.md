...
- For brokered DevStack MCP, follow **MCP Server Reference**: use `tool_search` if needed, call broker `retrieve_tools`, then pass the exact returned `name` through the exact returned `call_with` wrapper, such as `call_tool_read` when that is what discovery returns. Never synthesize a name from separate server or tool fields.
- Codex deltas: `mcpproxy` brokers `stack-core`, `trading-readonly`, and `task-control`, with DeepWiki through `stack-core`; direct stdio `agy-bridge`, native apps, and local/plugin MCPs remain separate surfaces. `mcp-run`, the retired `/v0/groups/agy/mcp` route, and Chunkhound are not current surfaces. Route details, retired names, snapshot semantics, and broker drift handling live in **MCP Server Reference**.
- The live v0.33-era broker and the isolated v0.51-derived sidecar are separate. In a proven sidecar session, authenticated management tools such as `quarantine_security` may be model-visible; preserve operation risk, scoped-token restrictions, and the explicit unquarantine gate. The candidate disables `/mcp/all`; do not invent or enable it as a normal or break-glass route.
...
- Consultation discipline, preamble, advisory treatment, stateful follow-up, Codex read-only sandbox, model/effort rules, and skip conditions live in **Ensemble Consultation**. Codex routes Gemini/Antigravity through directly exposed `agy-bridge` MCP tools: use `delegate` for bounded checks and `adversarial_review` for high-value review. If those direct tools are absent from the active session, report `ROUTE_UNAVAILABLE`; do not substitute `mcp-run`, the retired `/v0/groups/agy/mcp` route, or guessed broker names. GPT/Codex uses `codex-mcp-server` only for allowed high-stakes review, with `review` preferred.

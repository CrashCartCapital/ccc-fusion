...
- `PK-REFERENCE-EXTENDED` — Extended reference; trigger: reference; cost: ~2271 B; read `packs/AGENTS.md/PK-REFERENCE-EXTENDED--extended-reference.md`.
...
- `PK-MEMORY` — Memory surfaces; trigger: memory, checkpoint, memory, session recall, Hindsight, agent-session-search, Basic Memory; cost: ~6061 B; read `packs/AGENTS.md/PK-MEMORY--memory-surfaces.md`.
- `PK-MCP` — MCP routing; trigger: MCP broker, MCP discovery, tool routing, mcpproxy, MCPJungle, tool_search, retrieve_tools, call_tool_read, call_tool_write, call_tool_destructive; cost: ~19213 B; read `packs/AGENTS.md/PK-MCP--routing.md`.
- `PK-OMNIROUTE` — OmniRoute integration; trigger: OmniRoute, model routing, OmniRoute, omniroute; cost: ~5073 B; read `packs/AGENTS.md/PK-OMNIROUTE--integration.md`.
- `PK-HOOKS` — Hook verification; trigger: hook verification, runtime hooks, hooks, SafeExec; cost: ~4111 B; read `packs/AGENTS.md/PK-HOOKS--verification.md`.
- `PK-CONSULT` — Consultation mechanics; trigger: consultation, review, agy-bridge, codex-mcp-server; cost: ~14363 B; read `packs/AGENTS.md/PK-CONSULT--consultation.md`.
- `PK-WORKFLOW` — Workflow procedures; trigger: plan, debug, research; cost: ~9454 B; read `packs/AGENTS.md/PK-WORKFLOW--procedures.md`.
- `PK-RUNTIME-CLAUDE` — Claude runtime; trigger: Claude runtime; cost: ~3268 B; read `packs/AGENTS.md/PK-RUNTIME-CLAUDE--runtime.md`.
- `PK-RUNTIME-CODEX` — Codex runtime; trigger: Codex runtime; cost: ~6167 B; read `packs/AGENTS.md/PK-RUNTIME-CODEX--runtime.md`.
- `PK-GOOGLE-JULES` — Google Jules guidance; trigger: Jules; cost: ~6578 B; read `packs/AGENTS.md/PK-GOOGLE-JULES--remote-guidance.md`.
- `PK-PROJECT-LOCAL` — ccc-fusion local guidance; trigger: project-local guidance; cost: ~20211 B; read `packs/AGENTS.md/PK-PROJECT-LOCAL--ccc-fusion.md`.

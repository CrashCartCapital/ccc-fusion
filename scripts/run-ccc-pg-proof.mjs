...
const expectedWave5CliNames = [
  ...
  "prd command exit contract > stops reading typed operator context as soon as the byte limit is crossed",
  "prd command exit contract > refuses malformed typed operator context before calling packet freeze",
  ...
  "prd command exit contract > prints the read-only corpus manifest through the normal CLI",
  "prd command exit contract > writes a non-executable understanding review through one bounded native adapter",
  "prd command exit contract > refuses understanding admission when the selected provider model limits cannot be resolved",
  "prd command exit contract > test 44: a real chunked-lane mid-run failure through the CLI caller leaves the packet root byte-identical",
  "prd command exit contract > fn prd understand -- optionalUnderstandingFlags (design §6) > test 53: accepts the required flags plus --lane chunked",
  "prd command exit contract > fn prd understand -- optionalUnderstandingFlags (design §6) > test 53b: accepts the required flags plus --max-chunk-attempts",
  "prd command exit contract > fn prd understand -- optionalUnderstandingFlags (design §6) > test 54: rejects an unknown flag rather than silently dropping it",
  "prd command exit contract > fn prd understand -- optionalUnderstandingFlags (design §6) > test 54b: rejects a duplicate flag",
  "prd command exit contract > fn prd understand -- optionalUnderstandingFlags (design §6) > test 54c: rejects an odd arg count",
  "prd command exit contract > fn prd understand -- optionalUnderstandingFlags (design §6) > rejects --chunk-journal as not-yet-implemented, distinctly from an unknown flag",
  "prd command exit contract > fn prd understand -- optionalUnderstandingFlags (design §6) > rejects --resume as not-yet-implemented, distinctly from an unknown flag",
  "prd command exit contract > fn prd understand -- optionalUnderstandingFlags (design §6) > rejects --chunk-journal explicitly before generic option-count handling",
  "prd command exit contract > fn prd understand -- optionalUnderstandingFlags (design §6) > rejects --resume explicitly before generic option-count handling",
  "prd command exit contract > fn prd understand -- optionalUnderstandingFlags (design §6) > rejects an invalid --lane value",
  "prd command exit contract > generates a hash-bound execution plan without operator-authored policy JSON",
  ...
  "prd command exit contract > shows extracted PRD work and actionable verifier guidance when confinement is unavailable",
  "prd command exit contract > refuses import before project or importer residue when readiness has no admitted backend",
...
const wave6Commands = [
  ...
  },
  {
    ...
    command: ["pnpm", "--filter", "@fusion/engine", "exec", "vitest", "run", "src/__tests__/ccc-campaign-proof-admission.test.ts", "src/__tests__/ccc-campaign-proof-workflow.test.ts", "src/__tests__/ccc-campaign-local-git.real-git.test.ts", "--project=engine-default", "--silent=passed-only", "--reporter=dot"],
    vitestArgs: ["--reporter=json"],

...
CI blocks PRs on exactly four checks (`.github/workflows/pr-checks.yml`): **Lint, Typecheck, Build, Gate**. The Gate job runs the boot smoke (`scripts/boot-smoke.mjs`: CLI `--help` + a real `fn serve` answering `GET /api/health`) and `pnpm test:gate` (static process guards, curated `engine-core`, two general PostgreSQL canaries, focused CCC PRD fail-closed safety tests, and the CI-shape test). Everything else — the 4-way shards, the engine slow tier, the dashboard inventory guard — runs NON-BLOCKING in `.github/workflows/full-suite.yml` on push to main.

General engine gate membership is the explicit allow-list in `packages/engine/vitest.config.ts` (`engine-core` project). The separate `test:ccc-prd-safety` command is an intentionally small cross-package exception for the PRD-to-code safety boundary: verifier confinement, pre-effect refusal, operator status, and diagnostic privacy. Admission requires evidence of value (the test catches real regressions); tests never graduate in by default. A flaky general gate test is evicted by deleting its allow-list line — the eviction PR does not need the flaky test to pass. The whole `engine-core` project must stay under ~60s wall-clock.
...
**PostgreSQL gate policy:** `packages/core`'s general `test:pg-gate` intentionally runs only `task-lifecycle-e2e.pg.test.ts` and `handoff-to-review-atomicity.pg.test.ts`, preserving real-backend lifecycle and atomic-handoff canaries. It runs concurrently with `engine-core` after the static guards; the root script waits for **both** lanes and propagates either failure. The focused `test:ccc-prd-safety` lane then runs `ccc-prd-product-status.pg.test.ts` because uncertain-effect classification is part of the protected campaign boundary, not a general PG regression sample. Every other former PG gate member remains enabled and discovered by the non-blocking command `pnpm --filter @fusion/core test` (default config: `src/**/*.test.ts`, no PG quarantine exclusions). `scripts/__tests__/engine-vitest-gate-policy.test.mjs` pins the two general canaries and fails if any removed member is deleted, undiscoverable, or hidden by the default lane's script/config.
...
`pnpm test:full` runs each package's default test script with capped worker fanout (`FUSION_TEST_TOTAL_WORKERS=4 FUSION_TEST_CONCURRENCY=2 pnpm -r --no-bail --workspace-concurrency=2 test`). Do not casually raise worker counts; dashboard/jsdom and integration-heavy packages destabilize when oversubscribed. Use `VITEST_MAX_WORKERS=<n>` only for targeted package-level investigation.

`--no-bail` is deliberate and must not be dropped. Without it, pnpm's recursive run aborts at the first failing package with `ERR_PNPM_RECURSIVE_RUN_FIRST_FAIL`, and because `@fusion/engine` sorts before its reverse-dependents, a single engine failure silently prevented `cli`, `dashboard`, `desktop`, and `droid-cli` from executing at all — a "full suite" reading that actually covered 7 of 12 packages. On pnpm 10.33.0, `--no-bail` runs every package, prints a `Summary: N fails, M passes` line, and still exits nonzero via `ERR_PNPM_RECURSIVE_FAIL`, so the sweep stays honest. Note that `pnpm run --help` describes `--no-bail` as exiting 0 on failure; that text does not match 10.33.0's recursive-run behavior, so re-verify the exit code after any pnpm upgrade. For the same reason the three phases of `test:full` (recursive sweep, `engine test:slow`, `engine test:product-route`) are chained with status aggregation rather than `&&`: every phase runs, and the script exits nonzero if any phase failed. This is the local opt-in sweep only; CI runs `pnpm test:ci:shard` and keeps fail-fast semantics.

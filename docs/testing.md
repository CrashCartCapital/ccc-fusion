...
The current descendant repair has two required focused regressions before any consolidated acceptance:
...
These focused proofs use owned ephemeral loopback infrastructure only; no endpoint is a durable published surface. Core, engine, and CLI typechecks pass, and the global diff check passes. Task A, Task B, and draft `0037` remain development candidates pending complete Task 2 DDL, candidate freeze/commit, broader proof, and fresh council.

Task 2 owns new forward migration `0037_ccc_campaign_governance.sql`; Task 1 migration `0036_ccc_campaign_native_enforcement.sql` is immutable. Keep the Task 2 `0036→0037` upgrade proof in a dedicated `ccc-campaign-governance-migration.pg.test.ts` file so the closed Wave 5 migration command remains exactly three tests. Migration `0037` must not alter the structure, constraints, or identity of the Wave 5 custody tables inspected there (`ccc_prd_imports`, `ccc_prd_import_sources`, `ccc_prd_import_entities`), while Task 2 runtime code may use the Task 1 campaign counters and leases those tables intentionally expose. Because the applier records an immutable `0037` marker, the quarantine-only development draft must not be committed as an accepted migration or applied to a durable candidate database before the complete Task 2 DDL lands in the same final bytes. Its proof must cover registration in `schema-applier.ts`, `SCHEMA_BASELINE_VERSION === "0037"`, fresh `0000_initial.sql` parity, idempotent no-op behavior when `0037` runs after that fresh baseline, and direct rejection of partial audit/approval/effect campaign bindings through both typed writers and SQL constraints. Until the future closed `--wave 6` inventory exists, every focused Task 2 PostgreSQL run must use a task-owned disposable loopback fixture through explicit `FUSION_PG_TEST_URL_BASE`, must run with `FUSION_PG_TEST_SKIP` unset or not `1`, and must preserve reporter evidence of a nonzero executed-test count with zero skipped, pending, or todo tests; exit zero with a skipped PostgreSQL suite is a refusal, not proof. Add Task 2 names only after the exact inventory is complete; do not add a placeholder mapping.

...
- `task4-preprovider-admission` (`agent/ccc-fusion-task4-preprovider`, the accepted spine writer): tracked-clean at the frozen candidate; untracked read-only dependency-hydration symlinks into `wave-2`/`wave-3-retry` plus three candidate-local `@fusion/*` link views (`packages/plugin-sdk`, `packages/cli`, `packages/engine`) — intentional proof plumbing, retained so the accepted bytes remain provable in place; remove only when the worktree itself is retired.

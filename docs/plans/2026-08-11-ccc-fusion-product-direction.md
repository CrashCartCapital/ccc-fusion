...
- `glm-flash-latest` / GLM 5.3 Flash — routine implementation: the default worker route for normal-sized coding tasks once the spec is clear.
...
**How this is enforced, in plain terms:** the per-role assignment above is guidance for whoever builds a campaign's per-task routes, not a code-enforced rule. The CLI's `fn prd policy` step (`packages/cli/src/commands/prd-policy-routes.ts`) has no concept of "role" at all — it accepts one route per task and only checks that the route is well-formed (a real provider/model pair, no duplicates); it does not check a task against any approved list, and it would accept a task with the wrong role's route without complaint. The one check that is actually enforced in shipped code sits downstream, at settlement: `assertCccProviderAttemptEffectiveRoute` (`packages/core/src/ccc-campaign/provider-attempt.ts`, wired into the live Pi provider runtime) refuses to record a finished attempt whose OmniRoute-reported model does not match one of the terminal models that task's own route declared up front. That stops OmniRoute from silently swapping in a different model than promised, Luna included, but it cannot catch a human or a proposal author choosing the wrong role's route in the first place. The "golden" combo sealing and the golden Pi driver matrix's own admission test (both of which glm-flash-latest now passes) are an authoring-time and test-time safeguard, not a runtime gate — they verify a route resolves to what we think it does before it ever goes into a routes file, but nothing today stops an operator from bypassing that step.

...
This records the reviewed corrected plan-freeze that the later Task 5 Plan-Freeze Acceptance receipt accepted; implementation RED is active. Its immutable predecessors are accepted Task 4 product `bb04cff1c0301b55f7b3aa7c8729eb5bd93070c0` and documentation HEAD `29762606571c8a8aad02be01000eba06372045da`. The prior exact-byte candidate hashes `5a39…`, `c347…`, and `bd192…` are invalidated. The initial behavioral council returned FAIL P1 `1`; the initial architecture council returned FAIL P1 `3`, P0 `0`; their corrections were later reviewed and accepted in the receipt below.

- Ownership: the engine owns the fixed proof-host implementation and assets; the CLI is only its wrapper. In production, bootstrap and reverify the fixed contribution once in long-lived `InProcessRuntime`, construct exactly one `WorkflowTaskRuntime` after `TaskExecutor` through a public typed custom-node runner, and exclude the legacy executor from campaign work. The native CLI resolver uses persisted campaign context, semantic-task `protectedActionIds` derived/stored without a new migration, the exact active-action lease, native `preDispatch`, finite one-shot bounds, and native `CliSessionStore` settlement. The host authorizes only bound non-self-check proof declarations: exact admission/source/manifest/definition/input hashes; nonempty unique requirement IDs; bounded nonblank command, positive oracle, and negative controls. V1 command grammar is declaration-only `task <verify-prefixed-task> [--] [bounded canonical args...]`; it never checks or runs the Taskfile/command and refuses shell metacharacters, redirection, command substitution, wrappers, absolute executables, env assignment, or another executable. Thus real packet declarations can pass while `true`, `exit 0`, `sh -c`, `node -e`, and `rm -rf` refuse. Binding self-check remains evaluator-pass but workflow-non-authorizing.
- Queue: campaign routing is fail-closed union, not marker-and-custody intersection. An exact imported marker **or** persisted custody goes only to fenced `processDueWorkflowWorkItem`; marker-only must fail closed there, while custody-only uses the campaign route. Only marker-false plus custody-null remains ordinary. Claim only an `exactCandidate` whose `id`, `runId`, and `attempt` match under lease CAS; prove a stale or competing claimant cannot take it. Add no scheduler, parser, store, table, control plane, or database migration.
- Landing: freeze a campaign-only `LandRepoContext` that bypasses preexisting recovery, prune, dependency installation, local checkout/stash/push, provider merger/reviewer, retry rebuild, and every legacy finalization effect. Landing is object-only deterministic Git: strict `merge-tree --write-tree`, then `commit-tree` with fixed campaign/claim identity and timestamp, never a temporary worktree. Two snapshots are mandatory: admission snapshot remains authoritative; after merge-tree/commit-tree capture a self-write baseline that permits only the exact deterministic generated tree/commit objects and never blesses unrelated drift. Before intent and immediately before CAS, re-read and pin target/base/HEAD, cleanliness, target ref, normalized declared paths, target/git/common-dir/index realpaths and physical identities, every worktree checked-out-target state, and exact lease; unrelated drift refuses with zero CAS. Campaign strict overlap/path inspection propagates every Git read/parse failure with zero CAS and never reuses fail-open `detectMergeOverlap`; ordinary helper behavior stays unchanged.
- Merge routing and ref update: ProjectEngine resolves persisted campaign custody before merge-strategy or workspace dispatch. Affirmative custody routes only to campaign `runAiMerge`, even for `mergeStrategy=pull-request`; candidate/marker lookup uncertainty fails closed; ordinary no-custody is unchanged. Every TaskExecutor merge entry—the authoritative `requestMerge` primitive and the legacy merge seam—classifies persisted custody before `ensureWorkflowMergeBoundaryTask`; campaign graph uses the authoritative path and the legacy seam fails closed/skips mutation. Campaign requests retain untouched task identity, skip ensure/finalize, and require returned `campaignControlled` result bound to resolved custody. Caller-supplied marker alone never bypasses ordinary mutations; outer post-result `campaignControlled` suppression remains. Campaign `runAiMerge` returns before all legacy finalization: gate → deterministic Git-only squash → durable idempotent native `RunAuditEvent` intent → final custody/lease/path/ref recheck → CAS → atomic terminal audit plus exact approval consume → return. A forced crash after deterministic commit object but before intent leaves ref unchanged, approval claimed, and zero terminal/legacy/provider effects; replay regenerates and validates the exact same commit SHA before intent, with no orphan cleanup. Failed intent persistence means no CAS. It must not call `finalizeMerged`, group bookkeeping/PR sync, task branch/worktree cleanup, task lifecycle/`mergeDetails` mutation, `task:merged`, or push. ProjectEngine and TaskExecutor `requestMerge` honor a campaign-controlled `MergeResult` marker so outer legacy finalization, retry-reset, group promotion, `ensureWorkflowMergeBoundaryTask`, and `finalizeProvenAutoMergeTask` do not run; campaign graph/work-item lifecycle remains native. On restart, `ref == new` permits terminal audit and one approval consume only with the exact intent; `ref == expected` retries; any foreign ref enters manual hold and preserves the claim. Exact PostgreSQL oracle for a post-CAS terminal transaction failure is ref `== new`, zero terminal audit, approval claimed, and lease intact; restart performs no second CAS, then exactly one terminal audit and one consume/lease clear.

#### Frozen Task 5 RED Inventory
...
- RED evidence must additionally prove one real non-self-check declaration succeeds; malformed/stale/unbounded declarations and forbidden grammar refuse with zero subprocess/provider/action/hook effects; bootstrap failure makes no provider call; marker-only fails closed and custody-only stays on the campaign processor; real-Git two-snapshot custody rejects unrelated post-write drift before intent and CAS; PR-configured affirmative campaign custody makes zero PR/provider/legacy effects; custody lookup uncertainty fails closed; every TaskExecutor merge entry makes zero pre-request boundary mutation for campaign custody and legacy seam fails closed; caller marker alone cannot bypass ordinary mutation; object-only crash/replay preserves exact commit SHA; strict inspection failure makes zero CAS; intent-persistence failure makes no CAS; post-CAS PostgreSQL oracle holds then settles once; approval consumption is once-only; and a foreign ref holds while preserving its claim. A landing-level forbidden-effect recorder covers provider merger, reviewer, and stash seams across success and Git-landing refusal or recovery paths. ProjectEngine and TaskExecutor seam-specific mocks and assertions separately cover PR, workspace, finalizer, group, task-lifecycle, and boundary effects. Together they prove zero forbidden effects without adding an artificial cross-layer recorder.
- GREEN follows the ownership order above: fixed host/runtime, exact mixed-queue claim, resolver/settlement, frozen landing/gate, then intent/CAS reconciliation. Each success path remains local, deterministic, Git-only, and uses normal public readers for audit, approval, lease, and session state.
- REFACTOR reuses the native runner, TaskExecutor, TaskStore, `CliSessionStore`, native audit, and existing Git subprocess seam. Do not create a second merger, ref store, receipt table, Git state machine, scheduler, parser, or control plane; campaign strict overlap inspection is separate from unchanged ordinary `detectMergeOverlap`.
- Verification: focused engine unit and real-Git/loopback PostgreSQL suites named by the frozen REDs, including the two-snapshot real-Git, PR-configured custody, and every-TaskExecutor-entry pre-request-boundary REDs; `pnpm --filter @fusion/engine typecheck`; `pnpm --filter @fusion/engine build`; scoped lint; and `git diff --check`. Fresh exact-byte review passed after the behavioral P1 `1` and architecture P1 `3` corrections; the exact initial file map is frozen in the Task 5 Plan-Freeze Acceptance receipt below, while executable counts remain closed until RED cases exist.
- Done when: a fresh accepted candidate proves all five REDs green, real local Git proves all refusal/interruption/restart outcomes, and independent behavioral, static/build, and replacement architecture-council lanes return no P0/P1.
...
The corrected Task 5 plan freeze is accepted and implementation is authorized; RED is active. This is a plan/receipt transition only: no implementation or operator gate changed. The reviewed council candidate evidence remains fixed at plan SHA-256 `b33abe6c439278729c7790a862c0b816c643a2a5e7ffcd400cfdcbefc6cb7a52` and testing SHA-256 `0de24e2f511b87cd389594679d6ff8ad83d6065b9d87e3ae2a767a236ed7da72`; fresh behavioral and architecture councils both passed with P0 `0` and P1 `0`.

Custody-before-boundary is frozen for every TaskExecutor merge entry: authoritative `requestMerge`, the legacy merge seam, and `routeGraphMergeFailureToRetry` (approximately `executor.ts:10597`) must resolve persisted campaign custody before any merge-boundary mutation. The campaign path may not reach `ensureWorkflowMergeBoundaryTask` before that custody decision; the legacy path remains fail-closed and mutation-free for campaign custody.

The five frozen top-level RED names are exactly: `Task 5 RED: bootstraps one fixed proof host and one authoritative campaign runtime`; `Task 5 RED: mixed due queue preserves ordinary dispatch and claims campaign work only through the fenced processor`; `Task 5 RED: production native CLI resolver binds persisted route lease and restart settlement`; `Task 5 RED: refuses drift dirty overlap and undeclared paths before mutation`; and `Task 5 RED: reconciles interruption at each CAS boundary and consumes one exact approval`.

The initial RED file map is frozen: `packages/engine/src/__tests__/ccc-campaign-proof-admission.test.ts`; `packages/cli/src/commands/__tests__/ccc-native-proof-host.test.ts`; `packages/engine/src/__tests__/workflow-work-scheduler.test.ts`; `packages/engine/src/__tests__/workflow-work-processor.test.ts`; `packages/core/src/__tests__/postgres/taskstore-lifecycle.test.ts`; `packages/engine/src/__tests__/ccc-native-cli-public-route.real-pg.test.ts`; `packages/engine/src/__tests__/project-engine.test.ts`; `packages/engine/src/__tests__/workflow-merge-cancellation.test.ts`; new `packages/engine/src/__tests__/ccc-campaign-runtime-bootstrap.real-pg.test.ts`; and new `packages/engine/src/__tests__/ccc-campaign-git-integration.real-pg.test.ts`. Exact executable per-file test counts cannot be known until RED cases exist, so the proof-runner mapping remains closed and counts freeze before mapping.

### Task 5 Candidate Pre-Freeze Transition — 2026-07-26

Task 5 is a dirty repaired candidate, not accepted. Its accepted product predecessor is `bb04cff1c0301b55f7b3aa7c8729eb5bd93070c0`; its accepted plan-freeze documentation commit is `9a6ae1e41c62f6e4d2e7b5faf0f0a181972eccca`. A fresh candidate commit/tree and fresh behavioral/PostgreSQL, static/build, and native-Sol adversarial lanes remain mandatory.

Bootstrap P1 was accepted/fixed: RED expected running/rival but observed failed/null because bootstrap used an unowned direct transition. GREEN uses exact-candidate claim, owned bootstrap-refusal lease, and guarded transition; runtime real-PostgreSQL is `4/4`. Marker-only P1 was rejected: it mixed Task 3 generic-consumer pre-consumption rules with Task 5's sanctioned processor. The frozen Task 5 contract deliberately routes marker-only work to `processDueWorkflowWorkItem` to fail closed; terminal failure after exact claimed lease is restart-visible with zero provider/runner effect. Stale CLI typecheck P1 was partially accepted as proof-environment repair only: dashboard resolved old local `@fusion` links, then candidate-local current core/engine/i18n links restored proof. No product source, manifest, lockfile, install, or fetch changed. The five exact RED labels each now occur once.

Git landing now requires an exact active claimed approval lease immediately before CAS, observed CAS success, and a final exact ref reread before terminal effects; foreign-before/after-CAS races preserve approval/lease with zero terminal effects. Merge-retry custody lookup failure returns false with zero pre-dispatch mutation. Fresh focused proof is engine eight files `217/217`, core lifecycle `19/19`, CLI proof host `4/4`, total `240/240`; relevant core/engine/CLI/plugin-SDK typechecks and root lint pass; full typecheck `34/36`; full build passes including dashboard `7163` modules; built CLI top help and `prd --help` exit `0` and expose author/validate/compile. Manifest hashes are unchanged: package `cf1e924da8b13c1d6a4ed23b7e5cfb033b9e265a4676b8329050b2a9c6ba1755`, workspace `0e5f3ad808110908c6864d6fa02d05fe4a55d35eee75bf71815361f4c35118d1`, lock `09244dac5fdbc33029b5a44a9f7aca19c09de57ecb5c8547ca202eae6d34a7ab`. The 25 temporary package/plugin link views were removed and five copied proof directories were parked at `/tmp/ccc-task5-proof-hydration.xMWUPY`; only the separately documented root and core hydration links may remain before commit.

### Task 5 Second Candidate Repair Pre-Freeze — 2026-07-26
...
RED was resolved instead of rejected. GREEN exact case is `1/1`; paired pre-intent/intent rollback is `2/2`; full Git file is `17/17`; engine eight files are `219/219`; core lifecycle is `19/19`; built CLI proof-host is `4/4`; focused total is `242/242`; engine typecheck, changed-file ESLint, and diff check pass.

Pre-freeze Sol challenge found P0 `0`/P1 `1` only because docs falsely promised one physical cross-layer recorder. The safety intent is partially accepted, the artificial-recorder implementation is rejected, and this wording is corrected because granular native-seam assertions are behaviorally equivalent. There is no Task 5 acceptance; a new commit/tree and fresh behavioral/PostgreSQL, static/build, and adversarial lanes remain mandatory.

### Task 5 Retry Authority Repair Pre-Freeze — 2026-07-27

Candidate `0a45396a597f3c67e2fd9e9d36cb6e58cec4b050` is rejected. Behavioral and static lanes passed, but native Sol found P0 `0` and P1 `1`; root verified and accepted the finding: `routeGraphMergeFailureToRetry` swallowed campaign requester/missing-control errors, persisted, and returned `true`. The exact RED, `Task 5 RED: campaign graph retry refuses requester failure or missing exact custody authority`, resolved `true` instead of rejecting. GREEN rethrows on the campaign path before warning, persistence, or `true`; ordinary behavior is unchanged. REFACTOR removed the new explicit-`any` cast.

Exact proof is focused `1/1`, full workflow file `15/15`, engine eight files `220/220`, core `19/19`, CLI `4/4`, total `243/243`, engine typecheck, root lint, production ESLint, and `git diff --check` all passing. A child reported stale `219` by using a 16-test Git subtotal; root rejected that report and captured the exact `220` footer (`Git 17` plus `workflow 15`). Hydration proof views must be parked before commit. This dirty repair is not accepted: a new commit/tree and all three council lanes remain required.

### Task 5 Frozen Acceptance — 2026-07-27

Task 5 is accepted as its component and predecessor only. The accepted product commit is `fed32801b58a88cd5d037dbbd639afd00b910c22`; rejected parent is `0a45396a597f3c67e2fd9e9d36cb6e58cec4b050`; accepted tree is `d568d67b6d37b812df1d4f9695dc2696e7d00beb`. The total-versus-accepted plan is 39 paths, with path digest `4b025ed1af9f52e55b87aad7a5fcfc8a8270f9008a363594244dabd50609ce1c` and binary digest `a3ace83644fc1d6241f9e4d5db2dd193d4c6e8065928aab7af238bff58bb4580`.

The behavioral exact-replacement lane passed: engine `220`, core `19`, CLI `4`, total `243`, zero skips, and P0/P1/P2=`0`. The initially interrupted lane is rejected as incomplete. Static pretest/lint/typecheck `34/36`/build (dashboard `7163` modules)/built CLI top-level plus `prd` subcommand/diff/hash/non-ancestry all passed with P0/P1/P2=`0`. The literal standalone `prd` binary finding is rejected: declared bins are `agent-browser`, `fn`, and `fusion`; the contract is the built CLI subcommand, and Task 5 had no install or manifest authority. Adversarial review passed with P0/P1/P2=`0`.

The pre-receipt plan and testing hashes were respectively `32b80f9a13bade1183546beb2388283b4872f7c9a1313bd96309f6d145c8c127` and `981d7c5f21852de4e8faf405f2ae4ba7ae1e10c43a25e3441e1544bc4a703902`; full manifest hashes are unchanged. Status is root/core only. Wave 6 and Wave 7 are non-ancestors; earlier candidates are rejected. This is not literal full completion, provider approval, or live deployment. Next state: Task 6 RED for local mixed-provider synthetic work using deterministic fakes and native transports only.

### Task 6 Plan-Freeze Receipt — 2026-07-27

Task 6 is frozen against documentation descendant `0ef8eb84530b3c87a00be897ebb3e3a9f7fe3e5e` and accepted product parent `fed32801b58a88cd5d037dbbd639afd00b910c22`. The Spark inventory attempt was quota-blocked until August 1, so low-cost Terra substituted; native Sol, Terra, and root then performed the plan council. Root live-source adjudication set the verdict at P0 `0`, P1 `3`, with the three accepted blockers recorded in the amended Task 6 section above. The proposed `extensionId`-only controller-cache blocker is rejected because the map is local to one `executeNodeWithRetries` invocation and cannot cross node, semantic-task, route, visit, or execution-fence identity; within-node reuse is intended. This receipt accepts the corrected plan freeze and authorizes ordered RED work only; it does not accept a Task 6 product candidate or grant provider/live authority.
...
### Task 6 Replacement Candidate Frozen And Rejected On Dispatch Identity — 2026-07-27

The extension-identity replacement candidate was staged as tree `49a934a2cae38481da2b5ad9fc10fe8db561b8f8` (`22` paths, path digest `d04bf99a0c60ff932afe383194f1398cb8fb74758ea9f3fc98a2d5755f068fb7`, binary digest `f420fe8f18f874c8e7bb1e5a630a344f1520b8213694ba273f71ec0739f7ce17`) after fresh proof (extension `21/21`, engine `92/92`, real PostgreSQL/Git `28/28`, whole lint/build, standard gate `10/10` + `299/299` + `65/65`), then committed as `ce6274b74927fe46658dd5ec70d6aa46c63a348a`, parent `720e543f43e0010881f060331abdcbedef5c35cb`.

A fresh exact-byte council rejected that commit with P0 `0`, P1 `1`, P2 `0`: the scoped handler received a frozen `providerDispatch` descriptor but also the raw controller, whose `preDispatch` validated only `turnKey`/`transport`, so a forged `dispatchKey`/`providerId`/`modelId` could mint a different provider-attempt identity under the same authority. The behavioral and static lanes both passed `0/0/0` and remain development baseline evidence only. `execution_state` moved through `task6_dispatch_identity_seal_red_active`.

### Task 6 Accepted On Sealed Dispatch Candidate — 2026-07-27

RED captured the forged-`dispatchKey` failure signature; GREEN added `requireSealedWorkflowNodeProviderDispatch` and `createSealedWorkflowNodeProviderController` in `packages/engine/src/workflow-graph-executor.ts` (validate the candidate against the exact five sealed fields, then delegate the sealed original; refusal is `WORKFLOW_NODE_PROVIDER_REFUSED` before the raw controller) plus two focused tests (forged dispatch never reaches the raw controller; exact-value clones delegate the sealed object identity). The repair changed exactly two files.

Full reproof on the repair bytes passed: core campaign PostgreSQL `69/69`; importer `46/46`; engine provider/cancellation group `94/94` (new frozen total including the two seal tests); campaign execution/proof/Git `136/136`; real PostgreSQL/Git acceptance `28/28`; pretest, whole lint, core/engine typechecks, core/engine/workspace builds; standard gate core PostgreSQL `10/10`, engine `299/299`, CLI `65/65`; built-CLI user exercise proving the `0/1/2` exit contract with author-validate-compile round-trip; `git diff --check`; manifests unchanged.

The repair was committed as `7dc608a3537a79a67c347051de5e93186bf1c050`, parent `ce6274b74927fe46658dd5ec70d6aa46c63a348a`, tree `028142c1ff3492f2e3650141c9d71dbdba04eacc`. A fresh three-lane exact-byte council on that commit returned behavioral PASS `373/373` with P0/P1/P2 `0/0/0`, static/build/CLI PASS with P0/P1 `0/0` and one P2 (`prd --help` renders full top-level help; accepted contract met), and adversarial PASS with P0/P1 `0/0` and two P2 defense-in-depth notes (`reconcile` passthrough relies on core's `controllerToken` gate; pre-existing `contextPatch` merge cannot forge attempt identity). Task 6 is accepted at `7dc608a3537a79a67c347051de5e93186bf1c050`. Task 7 proof-runner work may now freeze the Task 6 inventory. `execution_state` is `task6_accepted_task7_proof_runner_active`.

### Speculative Wave 6/7 Final Disposition Ledger — 2026-07-27

Verified against accepted spine head `c8aa58471c2192c0e9fb2fe4509eb1c2d73005bb`: Wave 6 speculative `2f01f5f559f44cd60733d304d30976e973a5e894` and Wave 7 speculative `8be022a734fe47aa2deec2d6d9e494b31bef66a3` both exist locally and are both confirmed non-ancestors of the spine. Neither speculative production surface exists on the spine in any form: Wave 6's `packages/core/src/ccc-campaign/contract.ts` receipt/ceiling/drift/gate API and `packages/engine/src/ccc-proof/gate.ts` are absent (the spine's campaign governance is the distinct PostgreSQL-backed design in `canonical.ts`/`custody.ts`/`store.ts`/`provider-*.ts` plus migrations `0036`/`0037`); Wave 7's `packages/engine/src/ccc-integration/` reconciliation/injection/mutation-boundary modules are absent (the spine's merge/worktree safety lives in `merger.ts`/`merger-integration-worktree.ts`/`worktree-pool.ts`/`worktree-ref-sync.ts`). The `CF-016`/`CF-017` requirement tags appear nowhere in spine source or docs.

Per-idea disposition. Wave 6 (12 named test ideas): 10 rejected outright — canonical-bytes receipt identity, receipt idempotency/collision, malformed/stale receipt refusal, duplicate-proof-identity collision, operator-snapshot folding, no-side-effect verification, finite campaign ceilings (2 ideas), and per-field declared-drift admission (3 ideas) — their invariants and code vocabulary (`CF-RECEIPT-*`, `CF-CEILING-*`, `CF-DRIFT-*`, `evaluateCampaignCeiling`, `evaluateDeclaredDrift`, `foldOperatorSnapshot`, `evaluateCccProofGate`) have zero hits on the spine and depend on the discarded contract design. 2 selectively harvested in concept only: the protected-action decision-receipt pair ("rejects protected action without an exact decision receipt"; "admits only one exact action target packet decision and rejects replay or substitution") — the spine independently rebuilt the invariant that protected actions require exact, unforgeable, non-substitutable authorization before dispatch via the claim/lease/binding-provenance mechanism proven in `packages/engine/src/__tests__/ccc-campaign-execution.test.ts` (declared-action protectedness derivation, target-mismatch action-drift refusal, binding-provenance revalidation before transport effects). No further action needed.

Wave 7 (14 named test ideas): all 14 rejected — interruption reconciliation (6), merge/review/stash injection admission (3), and mutation-boundary manifest/sentinel policy (5) were never re-derived under any naming, and the one domain where the spine could have converged went the opposite way deliberately: `merger-integration-worktree.test.ts` proves the accepted policy for a dirty reused worktree is autostash-and-proceed (with refusal only when the autostash itself fails), affirmatively contradicting Wave 7's block-with-fixed-code `CF-MUTATION-INTEGRATION-DIRTY` design. Recorded as considered architectural divergence, not a gap.

Final disposition: both speculative commits are archived/discarded evidence with no outstanding harvest items. Neither may be replayed or cherry-picked. This ledger closes the Task 9 speculative-branch disposition requirement.

### Task 10 UP-N Local Boundary Review — 2026-07-27

Local-only upstream review, no fetch performed (fetch is operator-gated). Live evidence: `origin` is `https://github.com/CrashCartCapital/ccc-fusion.git` (fetch and push); `upstream` is `https://github.com/Runfusion/Fusion.git` with fetch configured and the push URL literally set to `DISABLED`, matching the plan's fetch-only contract. Roughly seventy `refs/remotes/upstream/*` branches are locally present from a full prior fetch; the shared `FETCH_HEAD` in the primary checkout's git common dir is dated 2026-07-23.

Bounded result: `git rev-list --count main..upstream/main` is `0`, and `git merge-base main upstream/main` equals the tip of `upstream/main` (`42fe154ab...`) — as of the locally present evidence, `upstream/main` is a strict ancestor of local `main`, so there is no adoptable upstream delta and no admissible UP-N range. Local `main` is one commit ahead (`d665917...`, the generated instruction-root install). Recording the empty range is the plan's own sanctioned outcome ("if no locally present range is admissible, record that bounded result instead of fabricating one").

Staleness caveat: the local upstream evidence is four days old (fetched 2026-07-23). Whether upstream has moved since cannot be concluded locally; a fresh `git fetch upstream` is the narrow operator-gated action that would refresh this review. No adoption branch was created; no upstream-facing action was taken.

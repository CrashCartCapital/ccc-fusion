...
- `packages/cli/src/commands/prd.ts` exposes built `prd author`, `validate`, and `compile` behavior with stable success/refusal/usage exits. The current descendant implements the designated native authoring path: `prd author` needs no proposal argument, uses bounded injected local transport/fakes, maintains an admitted previous sidecar, and refuses target/base/bounds/review drift. The deterministic proposal-file route remains a labelled compatibility fixture only, not the user path. This is integrated-green development evidence, not accepted Task A product.
...
- P1-04/P1-03 descendant repairs and Task A/Task B are integrated-green development evidence. The immediate order is now to complete and freeze full Task 2 governance in the one development-owned `0037`, then run broader candidate proof and fresh council before Tasks 3–10. Do not create `0038`. Because the migration applier records one immutable `0037` marker, the quarantine-only draft is development evidence: it must not be committed as an accepted migration or applied to a durable candidate database until the complete Task 2 governance DDL, fresh-schema parity, and upgrade proof are present in the same final `0037` bytes.
...
- P1-03 GREEN: the developing `0037_ccc_campaign_governance.sql` parks only explicit `unadmitted.v0` imports, pauses and triages only their ledger-owned tasks, and holds every runnable/running/retrying work item bound either directly by the import ledger or through one of those tasks. A simultaneously admitted v1 import remains active and runnable, and terminal work items are not resurrected. These bytes are focused development evidence only: the migration stays uncommitted and unaccepted until the complete Task 2 governance DDL occupies the same final `0037`; do not create `0038`.
...
- Current descendant evidence: Task A is integrated-green development evidence (engine native authoring `11/11`, CLI `2/2`, post-rebuild built CLI `12/12`, core contract `7/7`), not accepted product. The runner-reopen finding was accepted and repaired. Do not reopen Waves 1–5 or Task 1.
...
- Current descendant evidence: core, engine, and CLI typechecks pass and the global diff check passes. Task B and draft `0037` remain unaccepted until full Task 2 governance DDL, a candidate freeze/commit, broader proof, and fresh council.

...
- The original accepted documentation commit remains the execution-plan predecessor for Task 1. This Task 2 amendment preserves every accepted Task 1 byte, reserves `0037` for campaign governance, isolates its migration proof from the closed Wave 5 runner, explicitly advances fresh-schema baseline ownership to `0037`, and requires a fresh amendment review before Task 2 production edits. Any later discovery that invalidates a frozen contract must amend the plan, update the checkpoint, and receive a fresh review before dependent production edits continue.
